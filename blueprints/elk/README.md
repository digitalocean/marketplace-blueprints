...
If Kibana says "Kibana server is not ready yet.", wait until Elasticsearch has finished applying security (the Kibana droplet waits for the `kibana` user in cloud-init, but the UI can still warm up briefly after that).
...
## Stack details
- Kibana connects to Elasticsearch with the built-in `kibana` service account using `elasticsearch.username` / `elasticsearch.password` in `kibana.yml` (password is generated by Terraform and written on the Elasticsearch droplet under `/root/.digitalocean_passwords`).
